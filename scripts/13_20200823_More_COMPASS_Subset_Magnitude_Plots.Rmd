...
# parent_nodes_for_compass_runs <- c("4+", "8+")
...
```{r}
stim_labels <- c("Spike_1"="S1", "Spike_2"="S2", "NCAP" = "NCAP", "VEMP" = "VEMP")
...
plot_ifng_strat <- function(current_run, current_ylim=NA) {
  ...
  # The data is paired, but the violin plot + beeswarm shows the density better than a line plot
  # geom_line(aes(group=Individual), color="grey")
  current_plot <- ggplot(current_dat_long, aes(IFNg_Pos_or_Neg, BgCorr_Prop_Sum)) +
    theme_bw(base_size = 22) +
    geom_hline(yintercept = 0, linetype="dashed", alpha = 0.5) +
    geom_violin() +
    geom_beeswarm(color="black", size=0.5) +
    geom_errorbarh(data = current_dat_long %>% group_by(IFNg_Pos_or_Neg) %>% summarise(med = median(BgCorr_Prop_Sum)),
                   aes(y = med,
                       xmax = 1.5 + 0.35,
                       xmin = 1.5 - 0.35,
                       height = 0),
    ...
    scale_x_discrete(labels=c(c("IFNg_Pos" = "IFNg+", "IFNg_Neg" = "IFNg-")),
                       expand = c(0.3, 0.3)) +
...
ifng_stratified_plots <- map2(.x = c(CD4RunNames, CD8RunNames),
                              .y = list(c(-0.001, 0.011),
                                        c(-0.0025, 0.007),
                                        c(-0.004, NA),
                                        NA,
                                        c(-0.005, 0.0065),
                                        ...
                                        NA),
                              .f = plot_ifng_strat)
names(ifng_stratified_plots) <- c(CD4RunNames, CD8RunNames)

data.frame(run = names(ifng_stratified_plots),
           p.unadj = unlist(lapply(names(ifng_stratified_plots), function(n) {
             ifng_stratified_plots[[n]]$test_result$p.value
           }))) %>% 
...
plot_subset_vs_stim <- function(current_data, current_subset) {
  ...
  current_plot <- ggplot(current_data_for_plot, aes(STIM, BgCorrProp)) +
    theme_bw(base_size = 22) +
    geom_hline(yintercept = 0, linetype="dashed", alpha = 0.5) +
    # geom_point(position = position_jitter(width=0.12, height=0), size=0.3) +
    geom_violin() +
    # geom_quasirandom(method="quasirandom", size=0.3) +
    geom_beeswarm(color="black", size=0.3, cex=0.8) +
    geom_errorbarh(data = current_data_for_plot %>% group_by(STIM) %>% summarise(med = median(BgCorrProp)),
                   aes(y = med,
                       xmax = 1.5 + 0.47,
                       xmin = 1.5 - 0.47,
                       height = 0),
...
plot_subset_vs_stim_list <- lapply(cd4_booleanSubsets_gate_names, function(x) { plot_subset_vs_stim(current_data = cd4_dat, current_subset = x) })
names(plot_subset_vs_stim_list) <- cd4_booleanSubsets_gate_names
```

```{r}
current_subset <- cd4_booleanSubsets_gate_names[[1]]
ggplot_build(plot_subset_vs_stim_list[[current_subset]]$plot)$layout$panel_params[[1]]$y.range
annotation_df <- broom::tidy(plot_subset_vs_stim_list[[current_subset]]$test_result) %>%
  dplyr::filter(p.value < 0.05) %>% 
  mutate(y_pos = c(0.0283, 0.0333, 0.0308, 0.0085, 0.006, 0.0035),
         p.text = if_else(p.value < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.value, 3)))))
cd4_cd107a_plot <- plot_subset_vs_stim_list[[current_subset]]$plot +
...
annotation_df <- broom::tidy(plot_subset_vs_stim_list[[current_subset]]$test_result) %>%
  ...
  mutate(y_pos = c(0.0046, 0.0092, 0.0066),
         p.text = if_else(p.value < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.value, 3)))))
cd4_154_il2_tnf_plot <- plot_subset_vs_stim_list[[current_subset]]$plot +
...
annotation_df <- broom::tidy(plot_subset_vs_stim_list[[current_subset]]$test_result) %>%
  ...
  mutate(y_pos = c(0.002, 0.0076, 0.007),
         p.text = if_else(p.value < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.value, 3)))))
cd4_ifng_154_il2_tnf_plot <- plot_subset_vs_stim_list[[current_subset]]$plot +
...
cd8_dat <- gs_pop_get_count_with_meta(gs_cd8, subpopulations = c("8+", cd8_booleanSubset_gate_name)) %>% 
  dplyr::select(`SAMPLE ID`, Cohort, STIM, Population, Count) %>% 
  pivot_wider(names_from = Population, values_from = Count) %>% 
  mutate_at(vars(cd8_booleanSubset_gate_name), ~ . / `8+`) %>% 
  dplyr::select(-`8+`) %>% 
...
annotation_df <- broom::tidy(cd8_subset_vs_stim_out$test_result) %>%
  dplyr::filter(p.value < 0.05) %>% 
  mutate(y_pos = c(0.0281, 0.0339, 0.031, 0.0092, 0.0063, 0.0034),
         p.text = if_else(p.value < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.value, 3)))))
...
plot_subset_vs_cohort <- function(current_run, current_subset) {
  ...
  current_plot <- ggplot(current_data, aes(Cohort, !!as.symbol(current_subset))) +
    theme_bw(base_size = 22) +
      geom_hline(yintercept = 0, linetype="dashed", alpha = 0.5) +
      # geom_point(position = position_jitter(width=0.12, height=0), size=0.3) +
      geom_violin() +
      # geom_quasirandom(method="quasirandom", size=0.3) +
      geom_beeswarm(color="black", size=0.3, cex=1) +
      geom_errorbarh(data = current_data %>% group_by(Cohort) %>% summarise(med = median(!!as.symbol(current_subset))),
                     aes(y = med,
                         xmax = 1.5 + 0.47,
                         xmin = 1.5 - 0.47,
                         height = 0),
...
```{r, fig.width=5, fig.height=5}
plot_subset_vs_cohort("4_Spike_1", "IL2&!IL4/5/13&IFNg&TNFa&!IL17a&CD154&!CD107a")
plot_subset_vs_cohort("4_Spike_2", "IL2&!IL4/5/13&IFNg&TNFa&!IL17a&CD154&!CD107a")
plot_subset_vs_cohort("4_NCAP", "IL2&!IL4/5/13&IFNg&TNFa&!IL17a&CD154&!CD107a")
...
<!-- # Memory and activation of COMPASS subsets -->

<!-- Is this somewhat redundant with Fig 1C and D? -->
<!-- Plot the percent of memory and activated events out of COMPASS subset+ events -->
<!-- How to deal with DMSO/background? Do we just show DMSO as a separate boxplot? Is memory and activation affected by the experimental stimulation? -->

<!-- The presence of 5 different stims makes it somewhat difficult to represent this data concisely via boxplots. -->
