# This is a modified version of COMPASS's plot() function, based on the version here:
# https://github.com/malisas/COMPASS/blob/81a3cb22d55151eb88525feaede649eb6fb23cc9/R/plotMeanGamma.R
# https://github.com/malisas/COMPASS/blob/81a3cb22d55151eb88525feaede649eb6fb23cc9/R/pheatmap.R
...
convert_cytokine_annotations <- function(annotation, cytokine_annotation_colors=NA, dichotomize_by_cytokine=NA, dichotomize_by_cytokine_color=NA) {
  ...
  if(!is.na(dichotomize_by_cytokine)&!is.na(dichotomize_by_cytokine_color)) {
    # The color of all subsets which are positive for dichotomize_by_cytokine should be dichotomize_by_cytokine_color
    annot <- annot %>% 
      dplyr::mutate_all(~ if_else(!!as.symbol(dichotomize_by_cytokine) != "#FFFFFF" & . != "#FFFFFF",
                                              dichotomize_by_cytokine_color, .))
  }
...
heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation, annotation_colors, annotation_legend, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, row_annotation, row_annotation_legend, row_annotation_colors, cytokine_annotation, headerplot,polar=polar, cytokine_annotation_colors,cytokine_height_multiplier,draw_row_annotation_name=draw_row_annotation_name,dichotomize_by_cytokine=dichotomize_by_cytokine,dichotomize_by_cytokine_color=dichotomize_by_cytokine_color,...){
  grid.newpage()
  if(length(list(...))>0){
  ...
  if(!is.na(cytokine_annotation[[1]][1])){
    ...
    converted_cytokine_annotations = convert_cytokine_annotations(cytokine_annotation, cytokine_annotation_colors,
                                                                  dichotomize_by_cytokine, dichotomize_by_cytokine_color)
...
#' maximum functionality? Malisa note: in plot.COMPASSResult, this parameter indicates M (mean_gamma) column order, but here it seems to indicate cytokine_annotation label ordering by degrees. So it actually has two purposes.
#' @param cytokine_order_for_annotation is a vector containing the cytokine labels in the order you want them to appear in the cytokine annotation, from bottom to top. Columns are ordered by the number of cytokine-positive markers by default (see 'order_by_max_functionality' above). 'cytokine_order_for_annotation' will override 'order_by_max_functionality'.
#' @param staircase_cytokine_annotation If TRUE, achieve an aesthetically pleasing "staircase" effect in the cytokine annotation heatmap by re-ordering the subsets. Default is FALSE.
#' @param dichotomize_by_cytokine Re-orders the subsets so that all subsets positive for the specified subset are displayed last.
#' @param dichotomize_by_cytokine_color Subsets which are positive for dichotomize_by_cytokine will get colored this color in the cytokine annotation heatmap
#' @param draw_row_annotation_name Should the name of the row_annotation be displayed under the annotations?
#'
#' @return
...
my_pheatmap = function(mat, color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100), kmeans_k = NA, breaks = NA, border_color = "grey60", cellwidth = NA, cellheight = NA, scale = "none", cluster_rows = TRUE, cluster_cols = TRUE, clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "complete",  treeheight_row = ifelse(cluster_rows, 50, 0), treeheight_col = ifelse(cluster_cols, 50, 0), legend = TRUE, legend_breaks = NA, legend_labels = NA, annotation = NA, annotation_colors = NA, annotation_legend = TRUE, drop_levels = TRUE, show_rownames = TRUE, show_colnames = TRUE, main = NA, fontsize = 10, fontsize_row = fontsize, fontsize_col = fontsize, display_numbers = FALSE, number_format = "%.2f", fontsize_number = 0.8 * fontsize, filename = NA, width = NA, height = NA, row_annotation = NA, row_annotation_legend = TRUE, row_annotation_colors=NA, cytokine_annotation=NA, headerplot=NA, polar=FALSE, order_by_max_functionality=TRUE, cytokine_order_for_annotation=NA, cytokine_annotation_colors=NA, cytokine_height_multiplier=1, dichotomize_by_cytokine=NA, staircase_cytokine_annotation=FALSE, draw_row_annotation_name=TRUE, dichotomize_by_cytokine_color=NA, ...){
  ...
  # Previously in plot.COMPASSResult, the Count column was dropped and the cytokine_annotation matrix was turned into a data.frame
  if(!is.na(cytokine_annotation[[1]][1])){
    # Here drop=FALSE ensures that cytokine_annotation remains a data.frame even if there is only one column ("preserving" the type when subsetting)
    cytokine_annotation = cytokine_annotation[colnames(mat), ,drop=FALSE]
    
    # First deal with the cytokine_annotation label order (columns of the data.frame)
    
    if (is.na(cytokine_order_for_annotation) & order_by_max_functionality) {
      #order the columns by max functionality #Malisa note: here "functionality" is used to indicate "degrees"
    ...
    } else if(!is.na(cytokine_order_for_annotation)) {
      # order the columns by the fixed order in cytokine_order_for_annotation
      cytokine_annotation = cytokine_annotation[,cytokine_order_for_annotation,drop=FALSE]
    }
    
    # Now deal with the cytokine_annotation row order (rows of the data.frame). Changes will also get applied to mat column order.
    
    # If desired, achieve a "staircase" effect in the cytokine_annotation heatmap by re-ordering the order of rows of cytokine_annotation (and columns of mat)
    # Note that the rows will still be ordered by degrees afterwards, in the next chunk of code
    if(staircase_cytokine_annotation&!cluster_cols&is.na(headerplot)) {
      # Re-order the cytokine_annotation data.frame rows
      cytokine_annotation <- cytokine_annotation[rev(do.call(order, cytokine_annotation)),,drop=FALSE]
      mat <- mat[,rownames(cytokine_annotation)]
    }
    
    # order the rows of cytokine_annotation (and columns of mat) by degrees
    # Malisa note: this appears to override the mean_gamma functionality-based ordering in plot.COMPASSResult (order_by_max_functionality) in many cases
    ...
    # If dichotomize_by_cytokine is not NA, order the rows of cytokine_annotation (and columns of mat) so that all
    # subsets which are positive for dichotomize_by_cytokine are displayed last. Otherwise, the order remains untouched.
    if(!is.na(dichotomize_by_cytokine)) {
      cytokine_annotation <- cytokine_annotation[order(cytokine_annotation[,dichotomize_by_cytokine]),]
      mat <- mat[,rownames(cytokine_annotation)]
    }
  }
...
##' @export
my_plot.COMPASSResult <- function(x, y, subset=NULL,
                               threshold=0.01,
                               minimum_dof=1,
                               maximum_dof=Inf,
                               must_express=NULL,
