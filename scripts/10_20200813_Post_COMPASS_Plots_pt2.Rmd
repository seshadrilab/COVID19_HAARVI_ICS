...
```{r read in data}
gsPath <- here::here("out/GatingSets/20200815_HAARVI_ICS_GatingSet_AllBatches_with_COMPASS_Subsets_R4.0.3")
gs <- load_gs(gsPath)
...
cd4_dat <- gs_pop_get_count_fast(gs2, subpopulations = c("4+", cd4_booleanSubsets_gate_names)) %>% 
  left_join(pData(gs2) %>%
              dplyr::select(rowname, `SAMPLE ID`, STIM, Batch), by = c("name" = "rowname")) %>% 
  dplyr::select(`SAMPLE ID`, STIM, Batch, Population, Count) %>% 
...
#   dplyr::select(`SAMPLE ID`, STIM, Batch, Population, Count) %>%
cd8_dat <- gs_pop_get_count_fast(gs3, subpopulations = c("8+", cd8_booleanSubsets_gate_names)) %>% 
  left_join(pData(gs3) %>%
              dplyr::select(rowname, `SAMPLE ID`, STIM, Batch), by = c("name" = "rowname")) %>% 
  dplyr::select(`SAMPLE ID`, STIM, Batch, Population, Count) %>% 
  pivot_wider(names_from = Population, values_from = Count) %>%
...
<!-- # I get a weird Rcpp error when I run gs_pop_get_count_with_meta(gs, subpopulations = pop_paths_of_interest): -->
<!-- # Warning: stack imbalance in '<-', 2 then -1 -->
<!-- # and RStudio will crash sometimes, including when knitting the Rmd, so use gs_pop_get_count_fast instead -->
<!-- # Although I'm still getting weird errors sometimes -->
<!-- compass_mem_activation_dat_tmp <- gs_pop_get_count_fast(gs, subpopulations = pop_paths_of_interest) %>%  -->
<!--   left_join(pData(gs) %>% -->
<!--               dplyr::select(rowname, "SAMPLE ID", "STIM", "Cohort", "Age", "Sex", "Race_v2", "Hispanic?", -->
<!--                             "Days symptom onset to visit 1", "Batch"), by = c("name" = "rowname")) %>%  -->
...
<!--   # dplyr::filter(CD4_COMPASS_Pos != 0 & CD8_COMPASS_Pos != 0) # BWT23 and BWT22 have CD8_COMPASS_Pos = 0 -->

<!-- # Can I write out compass_mem_activation_dat_tmp to file and read it in to see if that gets rid of problems? -->
<!-- if(save_output) { -->
<!--   write.csv(compass_mem_activation_dat_tmp, here::here("processed_data/compass_mem_activation_dat_tmp.csv"), row.names = F) -->
<!--   # compass_mem_activation_dat_tmp_2 <- read.csv(here::here("processed_data/compass_mem_activation_dat_tmp.csv"), check.names = F) -->
<!--   # all.equal(compass_mem_activation_dat_tmp, compass_mem_activation_dat_tmp_2) -->
<!-- } -->
<!-- compass_mem_activation_dat_tmp_2 <- read.csv(here::here("processed_data/compass_mem_activation_dat_tmp.csv"), check.names = F) -->

<!-- # knitr::kable(compass_mem_activation_dat_tmp[1:6,]) -->
<!-- # gc() -->
<!-- # compass_mem_activation_dat_tmp %>%  -->
<!-- #   dplyr::filter(CD4_COMPASS_Pos == 0 | CD8_COMPASS_Pos == 0) %>%  -->
<!-- #   dplyr::select(`SAMPLE ID`, STIM, Cohort, CD4_COMPASS_Pos, CD8_COMPASS_Pos) -->

<!-- compass_mem_activation_dat <- compass_mem_activation_dat_tmp_2 %>% -->
