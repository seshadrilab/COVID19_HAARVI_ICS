...
crList2 <- lapply(crList,
                  function(cr) {
                    cr$data$meta$Cohort <- factor(ifelse(cr$data$meta$Cohort %in%
                                                c(NA, "NA", "Healthy control", "Healthy control 2017-2018"), "Healthy", cr$data$meta$Cohort),
...
crList.no_healthy <- lapply(names(crList2),
                  function(cr_name) {
                    ...
                    # The following was not the case during a re-analysis, so had to filter using exact SAMPLE ID text matching
                    # instead of indices
                    ...
                    not_healthy_sample_ids <- cr$data$meta$`SAMPLE ID`[which(cr$data$meta$Cohort != "Healthy")]
                    cr$data$meta <- cr$data$meta %>% 
                      dplyr::filter(`SAMPLE ID` %in% not_healthy_sample_ids) %>% # [not_healthy_idx,] %>% 
                      mutate(Cohort = factor(Cohort, levels = c("Non-hospitalized", "Hospitalized")))
                    cr$fit$mean_gamma <- cr$fit$mean_gamma[which(rownames(cr$fit$mean_gamma) %in% not_healthy_sample_ids),]

                    cr$data$n_s <- cr$data$n_s[which(rownames(cr$data$n_s) %in% not_healthy_sample_ids),]
                    cr$data$n_u <- cr$data$n_u[which(rownames(cr$data$n_u) %in% not_healthy_sample_ids),]
                    cr$data$counts_s <- cr$data$counts_s[which(names(cr$data$counts_s) %in% not_healthy_sample_ids)]
                    cr$data$counts_u <- cr$data$counts_u[which(names(cr$data$counts_u) %in% not_healthy_sample_ids)]
...
fs_pfs_df_with_manifest <- lapply(c(CD4RunNames, CD8RunNames), function(n) {
  ...
  mutate(Cohort = factor(ifelse(Cohort %in%
                                                c(NA, "NA", "Healthy control", "Healthy control 2017-2018"), "Healthy", Cohort),
...
```{r, fig.width=4, fig.height=4}
supp_tbl_1 <- read.csv(here::here("data/T-cell Supplemental Table 1_20210126_MS.csv"), check.names = F, stringsAsFactors = F)
...
if(save_output) {
  ...
  print((fs_vs_age_plot_list$S1_4 | fs_vs_age_plot_list$S2_4 | fs_vs_age_plot_list$N_4 | fs_vs_age_plot_list$E_4)  /
  ...
  pdf(file=here::here("out/PostCompassPlots/FS_Plots/Fig5HIJK_CD8_FS_vs_Demographics.pdf"), width=14, height=14,
    onefile = TRUE, bg = "transparent", family = "Arial", fonts = "Arial")
  print((fs_vs_age_plot_list$S1_8 | fs_vs_age_plot_list$S2_8 | fs_vs_age_plot_list$N_8 | fs_vs_age_plot_list$E_8)  /
...
if(save_output) {
  pdf(file=here::here("out/PostCompassPlots/Magnitude_Plots/FigS5A_CD4_S1_COMPASS_Magnitudes_vs_Cohort.pdf"), width=12, height=7,
    onefile = TRUE, bg = "transparent", family = "Arial", fonts = "Arial")
  print(bg_corr_dotplots[["4_Spike_1"]]$Dotplot)
  dev.off()
...
### Re-make magnitude plots as boxplot with heatmap categories legend
...
if(save_output) {
  ...
  print(grid.draw(bg_corr_boxplots[["4_Spike_1"]]$Plot))
  dev.off()
  ...
  cairo_pdf(file=here::here("out/PostCompassPlots/Magnitude_Plots/FigS5B_CD4_S2_COMPASS_Magnitudes_vs_Cohort_Boxplots.pdf"), width=11.5, height=7.2,
    onefile = TRUE, bg = "transparent", family = "Arial")
  print(grid.draw(bg_corr_boxplots[["4_Spike_2"]]$Plot))
  dev.off()
  ...
  cairo_pdf(file=here::here("out/PostCompassPlots/Magnitude_Plots/FigS5C_CD4_N_COMPASS_Magnitudes_vs_Cohort_Boxplots.pdf"), width=14, height=7.2,
    onefile = TRUE, bg = "transparent", family = "Arial")
  print(grid.draw(bg_corr_boxplots[["4_NCAP"]]$Plot))
  dev.off()
  ...
  cairo_pdf(file=here::here("out/PostCompassPlots/Magnitude_Plots/FigS5D_CD4_E_COMPASS_Magnitudes_vs_Cohort_Boxplots.pdf"), width=3, height=7.2,
    onefile = TRUE, bg = "transparent", family = "Arial")
  print(grid.draw(bg_corr_boxplots[["4_VEMP"]]$Plot))
  dev.off()
}
```
